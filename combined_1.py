	...
	# Note: Do not make this function lenghty, do all the processing outside this callback function
		try:
			...
			for i in range(len(right_top)):
				...
				# if self.ids:
				# global direction
				# rospy.loginfo(f"Going {direction}")
...
def deploy_picker(ids, setpoints_arr):
	...
	for i in range(len(box_loc_x_f)):
		...
		print("In Position ", i)
		# collect_gps = 0
		offboard2.reach_position2(box_loc_x_f[i]-drone_1_x, box_loc_y_f[i]-drone_1_y, 5, 0.1, 0.1, 0.1, 100)
		offboard2.reach_position2(box_loc_x_f[i]-drone_1_x, box_loc_y_f[i]-drone_1_y, 0, 0.1, 0.1, 0.1, 20)
