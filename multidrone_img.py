	...
	# Note: Do not make this function lenghty, do all the processing outside this callback function
		try:
			...
			for i in range(len(right_top)):
				...
				# if self.ids:
				if (int(self.centerx[i]) in range(190, 210) and collect_gps==0):
					collect_gps = 1
					...
					box_pos_x.append(box_loc_x)
					box_pos_y.append(box_loc_y)
...
if __name__ == '__main__':
	...
	# positions = ((-1, 1, 0), (-1, 1, 3), (-1, 16.2, 3), (-1, 16.2, 0))

	# for i in range(len(positions)):
	# 	if i==0:
	# 		offboard.reach_position(positions[i][0]+1, positions[i][1]-1,
	# 							positions[i][2], 0.5, 0.5, 0.5, 30)
			
	# 	if i>0:
	# 		offboard.reach_position(positions[i][0], positions[i][1],
	# 							positions[i][2], 0.5, 0.5, 0.5, 30)
		
	# # for i in range (10):
	# 	# offboard.set_gripper(True)

	# positions = ((6.5, 0, 3), (9, 0, 3), (9,0,-1))

	# for i in range(len(positions)):
	# 	offboard.reach_position(positions[i][0], positions[i][1],
	# 						positions[i][2], 0.5, 0.5, 0.5, 30)

	# # offboard.set_gripper(False)

	# positions = ((9, 0, 3), (0, 0, 3), (0,0,0))
